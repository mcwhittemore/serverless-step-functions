...
const toMermaid = require('./mermaid/to-mermaid');
...
class ServerlessStepFunctions {
  constructor(serverless, options, v3Api) {
    ...
    this.commands = {
      stepf: {
        mermaid: {
          usage: 'convert step functions into mermaid diagrams',
          lifecycleEvents: ['run'],
          options: {
            output: {
              usage: 'Destination of the mermaid diagrams',
              required: true,
              type: 'string',
            }
          }
    ...
    this.hooks = {
      'stepf:mermaid:run': () => BbPromise.bind(this)
        .then(this.yamlParse)
        .then(() => this.mermaid(writeFile)),
  ...
  mermaid(writeFile) {
    return BbPromise.bind(this).then(() => {
      ...
      const stateMachines = Object.keys(this.serverless.service.stepFunctions.stateMachines);

      if (stateMachines.length === 0) {
        throw new Error('No step functions to create diagrams for');
      }
      ...
      for(const sm of stateMachines) {
        output += `## ${sm}\n`;
        output += '```mermaid';
        output += toMermaid(this.serverless.service.stepFunctions.stateMachines[sm]);
        output += '```\n';
      }
      
      return writeFile(this.options.output, output);
    });
  }
